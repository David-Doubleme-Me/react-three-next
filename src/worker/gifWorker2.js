...
})(
  {
    1: [
      function (require, module, exports) {
        let NeuQuant = require('./TypedNeuQuant.js')
        let LZWEncoder = require('./LZWEncoder.js')
        function ByteArray() {
          this.page = -1
          this.pages = []
          this.newPage()
        ...
        GIFEncoder.prototype.indexPixels = function (imgq) {
          let nPix = this.pixels.length / 3
          this.indexedPixels = new Uint8Array(nPix)
          let k = 0
          for (let j = 0; j < nPix; j++) {
            let index = this.findClosestRGB(this.pixels[k++] & 255, this.pixels[k++] & 255, this.pixels[k++] & 255)
        ...
        GIFEncoder.prototype.ditherPixels = function (kernel, serpentine) {
          ...
          for (let y = 0; y < height; y++) {
            ...
            for (
              ...
              idx *= 3
              let r2 = this.colorTab[idx]
              let g2 = this.colorTab[idx + 1]
              let b2 = this.colorTab[idx + 2]
              let er = r1 - r2
              let eg = g1 - g2
              let eb = b1 - b2
              for (
        ...
        GIFEncoder.prototype.findClosestRGB = function (r, g, b, used) {
          ...
          for (let i = 0, index = 0; i < len; index++) {
            let dr = r - (this.colorTab[i++] & 255)
            let dg = g - (this.colorTab[i++] & 255)
            let db = b - (this.colorTab[i++] & 255)
        ...
        GIFEncoder.prototype.writePixels = function () {
          let enc = new LZWEncoder(this.width, this.height, this.indexedPixels, this.colorDepth)
          enc.encode(this.out)
        }
    ...
    2: [
      function (require, module, exports) {
        ...
        let BITS = 12
        let HSIZE = 5003
        let masks = [0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535]
